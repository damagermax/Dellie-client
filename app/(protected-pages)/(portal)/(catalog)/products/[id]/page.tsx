...
export default function ProductDetailPage({
  ...
  const canViewProduct = hasAnyPermission([
    StorePermission.PRODUCTS_VIEW,
    StorePermission.PRODUCTS_MANAGE,
  ]);
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetProductQuery(id, { skip: !id || !ready || !canViewProduct });
  ...
  const canToggleDescription = Boolean(
        ...
        <main className="border-r border-gray-200 bg-white">
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0 hidden md:block font-semibold">
                  <div className="flex flex-wrap items-center gap-2">
                    <h1 className="text-2xl   text-gray-700 truncate">
                      {product.name}
                    </h1>
          ...
          <section className="grid sm:gap-6 md:px-5 md:py-6 lg:grid-cols-[200px_minmax(0,1fr)]  cursor-pointer ">
            <button
              ...
              className="group w-[20%] lg:w-full cursor-pointer disabled:cursor-default disabled:opacity-100 relative aspect-square overflow-hidden rounded-sm border  border-[#2d837d] sm:border-gray-300 hover:border-[#2d837d] text-left outline-none transition  focus-visible:ring-2 focus-visible:ring-gray-950"
            >
              {showImagePlaceholder ? (
                <ProductImagePlaceholder label="Product image" />
              ) : (
                <img
                  className="h-full  w-full hover:p-1 object-cover"
                  src={imageUrl}
              ...
              {canManageProduct && (
                <span className="absolute inset-x-0 bottom-0 bg-[#2d837d] px-3 py-2 text-center text-xs font-medium text-white sm:opacity-0 transition group-hover:opacity-100 group-focus-visible:opacity-100">
                  Manage media
                </span>
              )}
            </button>
            ...
            <div className="min-w-0">
              <DetailGrid
                items={[
                  ...
                  {
                    label: INVENTORY_TYPES.includes(product.type)
                      ? "Available"
                      : "Inventory",
                    value: INVENTORY_TYPES.includes(product.type)
                  ...
                  },
                  {
                    ...
                    value: INVENTORY_TYPES.includes(product.type)
                      ? formatMoney(product.inventory?.summary?.inventoryValue)
                      : "Not tracked",
                  },
                ]}
              />
...
function Pricing({ product }: { product: ProductDetail }) {
  ...
  const margin = product.sellingPrice
    ? `${((profit / Number(product.sellingPrice)) * 100).toFixed()}%`
    : "0%";
  return (
    ...
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryCard
          ...
          value={formatQuantity(
            summary.availableQuantity ?? product.availableStock,
          )}
        />
        <SummaryCard
          label="FIFO Value"
        ...
        <SummaryCard
          label="Average Cost"
          value={formatMoney(summary.averageCost)}
        />
        <SummaryCard
          label="Active Batches"
          value={formatQuantity(summary.activeBatches)}
        />
        <SummaryCard
          label="Expired Qty"
          value={formatQuantity(summary.expiredQuantity)}
          muted={!summary.expiredQuantity}
        />
        <SummaryCard
          label="Expiring Soon"
          value={formatQuantity(summary.expiringSoonQuantity)}
          muted={!summary.expiringSoonQuantity}
        />
      </div>
      ...
      <Panel>
        ...
        {locations.length ? (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {formatQuantity(location.committed)} committed
                  </p>
                </div>
                <p className="text-sm font-semibold text-gray-900">
                  {formatQuantity(location.available)} available
                </p>
              </div>
            ))}
          </div>
          ...
          <Empty
            className="py-6"
            description="No inventory has been received yet."
          />
        )}
      </Panel>
    </div>
      ...
      <Panel>
        ...
        <div className="mt-4 space-y-3">
          ...
          <Metric
            ...
            value={product.conversionRule || "-"}
          />
          <Metric
            label="Available to sell"
            value={formatQuantity(product.availableStock)}
          />
        </div>
      </Panel>
      <Panel>
        ...
        <div className="mt-4 space-y-3">
          <Metric
            ...
            value={formatQuantity(sourceSummary.availableQuantity)}
          />
          <Metric
            label="Source value"
            value={formatMoney(sourceSummary.inventoryValue)}
          />
          <Metric
            label="Active source batches"
            value={formatQuantity(sourceSummary.activeBatches)}
          />
        </div>
      </Panel>
    </div>
    ...
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-3">
        <SummaryCard
          ...
          value={formatQuantity(product.bundleAvailability)}
        />
        <SummaryCard
          label="Components"
          value={formatQuantity(components.length)}
        />
        <SummaryCard
          label="Bundle Price"
          value={formatMoney(product.sellingPrice)}
      ...
      <Panel>
        ...
        {components.length ? (
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {component.sku || component.type}
                    </p>
                  </div>
                </div>
                <div className="text-right text-sm">
                  <p className="font-semibold text-gray-900">
                    {formatQuantity(component.quantityRequired)} needed
                  </p>
                  <p className="text-gray-500">
                    {formatQuantity(component.availableQuantity)} available
                  </p>
                </div>
              </div>
            ))}
          ...
          <Empty
            className="py-6"
            description="No bundle components have been configured."
          />
        )}
      </Panel>
    </div>
...
function Activity({ product }: { product: ProductDetail }) {
  const emptyText =
    product.type === "SERVICE"
      ? "Service sales and refunds will appear here."
      : product.type === "BUNDLE"
        ? "Bundle sales and returns will appear here."
        : "Purchases, sales, fulfillments, returns, and adjustments will appear here.";
  return (
    <Panel>
  ...
  if (!batches.length) {
    return (
      <Panel>
        <Empty
          className="py-10"
          description="No inventory batches have been created yet."
        />
      </Panel>
    );
  }
...
function SummaryCard({
  ...
  return (
    <div
      className={`rounded-sm border border-gray-200 bg-white p-4 ${muted ? "opacity-60" : ""}`}
    >
      <p className="text-xs font-medium uppercase tracking-[0.12em] text-gray-400">
        {label}
      </p>
...
function Panel({
  ...
  return (
    <div
      className={`rounded-md border border-gray-200 bg-white p-5 ${className}`}
    >
      {children}
    </div>
  );
...
function formatQuantity(value: unknown) {
  const amount = Number(value || 0);
  return new Intl.NumberFormat("en-GH", { maximumFractionDigits: 6 }).format(
    amount,
  );
}
