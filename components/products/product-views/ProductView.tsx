...
export default function ProductView({ view, query, onQueryChange }: ProductViewProps) {
  const { data: productData, isLoading: loadingProducts, isFetching } = useGetProductsQuery(query);
  ...
  return (
    <section className="  h-[calc(100vh-12rem)]  ">
      ...
      {loadingProducts ? <MobileListShimmer /> : <ProductsMobileList products={mobileList.items} />}
